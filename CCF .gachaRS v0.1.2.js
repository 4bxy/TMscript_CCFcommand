...
(function () {
  ...
  const DB_CACHE_META_KEY = '__CCF_GACHARS_DB_META__';

  // 再発火防止
  const PROCESSED_KEY = '__CCF_GACHARS_PROCESSED__';
  ...
  // ★ここが修正点：発言者名の中に COMMAND が混入していたら確実に除去
  function cleanSpeakerName(raw, messageText) {
    let s = normalizeName(raw);

    // 「4bxy - 今日 1:41」形式
    if (s.includes(' - ')) s = normalizeName(s.split(' - ')[0]);

    // primaryに本文が混入して「4bxy.gachaRS」「4bxy .gachaRS」等になるケースを除去
    // COMMAND が含まれていたら、その手前を採用（区切りが無くても確実に切る）
    ...
    // 本文と同じ/コマンドそのものなら無効扱い
    if (!s || s === messageText || s === COMMAND) return '';
    return s;
  }
  ...
  function extractSpeakerName(container, messageText) {
    ...
    let name = cleanSpeakerName(primaryRaw, messageText);
    if (name) return name;

    // 2) フォールバック：container内の短いテキスト候補から拾う
    ...
    for (const el of elems) {
      ...
      const c = cleanSpeakerName(t, messageText);
  ...
  async function runGacha5(ownerName) {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      await moveOneTokenByNameToXY(tokenName, pos.x, pos.y, beforeSet);
    }
  ...
  scanExistingOnce();
  startObserver();
  initDb().then((ok) => ok && console.log(`${LOG_PREFIX} DB先読み完了✅`));
})();
