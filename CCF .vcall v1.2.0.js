 ...
 * - v1.2.0: 召喚完了後に「魔物知識の値／弱点値(弱点内容)」をチャットへ自動お知らせ。
 *   魔神化の固定値上昇表は個体ごとに抽選されるため、弱点値は実際に召喚した
 *   個体の memo から取得する（同値はまとめ、差が出た個体だけ連番付きで別行）。
 *   お知らせは新規発言ではなく、従来の self-delete（検知1000ms後の「編集→保存」）で
 *   保存する文字列を空文字からお知らせテキストに変えることで出す。
 *   そのため全個体のレコードは貼り付け前に作り切って値を確定させている。
 *   上書きに失敗したときだけ、別メッセージとして投稿する。
 */
...
(function () {
  ...
  // 編集ドロワのテキストを newText に差し替えて保存する
  // newText が '' なら空保存＝実質削除（従来の self-delete）
  async function replaceAndSaveViaEditTab_NoDelay(newText) {
    for (let i = 0; i < 30; i++) {
  ...
  const selfEditScheduled = new WeakMap();
  const selfEditFinished = new WeakSet();

  // 自分のコマンド行を「ペン→編集→保存」で書き換える（即時実行版）
  // 成功なら true。ペンや編集ドロワが取れなければ false
  async function editOwnMessage(node, newText) {
    if (!(node instanceof HTMLElement)) return false;
    if (selfEditFinished.has(node)) return false;
    if (!isInDocument(node)) return false;

    const penOk = await clickEditPenForMessage(node);
    if (!penOk) {
      console.debug(LOG, 'self-edit: ペン要素が見つからない/クリック失敗');
      return false;
    }
    const saveOk = await replaceAndSaveViaEditTab_NoDelay(newText);
    if (!saveOk) {
      console.warn(LOG, 'self-edit: 編集ドロワ操作失敗（textarea/保存 未検出）');
      // 開きっぱなしのドロワを閉じてから呼び元のフォールバックに任せる
      closeAnyMenuByEsc();
      return false;
    }
    selfEditFinished.add(node);
    return true;
  }

  function scheduleSelfEdit(node, newText, afterMs = 1000) {
    if (!(node instanceof HTMLElement)) return;
    if (selfEditFinished.has(node)) return;
    if (selfEditScheduled.has(node)) return;

    const id = setTimeout(async () => {
      selfEditScheduled.delete(node);
      const ok = await editOwnMessage(node, newText);
      if (ok) {
        console.log(LOG, newText === ''
          ? `self-edit: ${afterMs}ms後に空保存完了（実質削除）`
          : `self-edit: ${afterMs}ms後にコマンド行を魔物知識で上書き`);
        return;
      }
      // 上書きできなかったときだけ、お知らせを別メッセージとして投稿して取りこぼしを防ぐ
      if (newText !== '') {
        console.warn(LOG, 'コマンド行の上書きに失敗。魔物知識を別メッセージで投稿します');
        postChatMessage(newText);
      }
    }, afterMs);

    selfEditScheduled.set(node, id);
    console.log(LOG, `self-edit を ${afterMs}ms 後にスケジュール`);
  }

  function scheduleSelfDeletion(node, afterMs = 1000) {
    scheduleSelfEdit(node, '', afterMs);
  }
  ...
  // ============================================================
  // ====== 魔物知識のお知らせ（チャット投稿） =====================
  //   - 召喚が全部終わってから「魔物名 魔物知識の値／弱点値(弱点内容)」を投稿
  //   - 魔神化(-mN)の固定値上昇表は個体ごとに抽選されるため、弱点値は
  //     実際に召喚した個体の memo から取得する
  // ============================================================

  // チャット入力欄へ投稿（.execute / .cur / .ccfai と同一方式）
  function postChatMessage(message) {
    const inputBox = document.querySelector('textarea[placeholder="メッセージを入力"]');
    if (!inputBox) {
      console.warn(LOG, 'チャット入力欄が見つかりませんでした（魔物知識の投稿をスキップ）');
      return;
    }
  ...
  // memo の「魔物知識:」の値を抜き出す（例: "6／11(魔法+2)" "3／(なし)"）
  // 区切りは全角スペース・タブ・改行（HTML版 extractFromMemo と同じ）
  // 行が無い／値が空の個体は null（＝お知らせ対象外）
  function extractKnowledgeFromMemo(memo) {
    if (typeof memo !== 'string') return null;
    const m = memo.match(/魔物知識:([^\n　\t]*)/);
    if (!m) return null;
    const v = m[1].trim();
    return v === '' ? null : v;
  }

  // 個体ごとの結果を同値でまとめて行文字列にする
  //  - 全個体が同値なら連番なしの1行
  //  - 値が分かれたときだけ、その個体の連番(A・B…)を付けて別行にする
  function buildKnowledgeLines(displayBase, entries) {
    const groups = []; // { knowledge, letters: [] }

    entries.forEach(e => {
      if (e.knowledge == null) return;
      let g = groups.find(gg => gg.knowledge === e.knowledge);
      if (!g) {
        g = { knowledge: e.knowledge, letters: [] };
        groups.push(g);
      }
      if (e.letter) g.letters.push(e.letter);
    });

    if (groups.length === 0) return [];
    if (groups.length === 1) return [`${displayBase} ${groups[0].knowledge}`];

    return groups.map(g => {
      const suffix = g.letters.length > 0 ? g.letters.join('・') : '';
      return `${displayBase}${suffix} ${g.knowledge}`;
    });
  }
  ...
  // 召喚する全個体のレコードを、貼り付けを始める前にまとめて作る。
  // 剣の欠片・魔神化の抽選はここで確定するので、魔物知識の値もこの時点で確定する。
  //   → コマンド行の書き換え（従来の self-delete と同じ 1000ms 後）に間に合う。
  // 戻り値: { records, knowledgeText }（knowledgeText は取得できなければ ''）
  function buildVCallRecords(baseRec, baseName, initiative, count, shardCount, demonizeStage) {
    const records = [];
    const knowledgeEntries = []; // { letter, knowledge } 個体ごとの魔物知識
    ...
    for (let i = 0; i < count; i++) {
      ...
      // 魔物知識のお知らせ用に、この個体の確定値を控える
      // （魔神化の固定値上昇表はこの個体だけの抽選結果なので、個体ごとに取る）
      knowledgeEntries.push({
        letter: count > 1 ? suffixLetter(i) : '',
        knowledge: extractKnowledgeFromMemo(cloned.data.memo),
      });

      records.push(cloned);
    }

    let displayBase = baseName || '';
    if (shardCount > 0 && !displayBase.includes('†')) displayBase += '†';
    if (demonizeStage > 0) displayBase += '◎'.repeat(demonizeStage);

    const knowledgeLines = buildKnowledgeLines(displayBase, knowledgeEntries);
    if (knowledgeLines.length === 0) {
      console.warn(LOG, '魔物知識の値がmemoから取得できなかったため、お知らせは行いません:', baseName);
    }

    return { records, knowledgeText: knowledgeLines.join('\n') };
  }

  // 作り置きしたレコードを1体ずつクリップボード経由で貼り付ける
  async function pasteVCallRecords(records, shardCount, demonizeStage) {
    for (let i = 0; i < records.length; i++) {
      const cloned = records[i];
  ...
  function handleVCall(fullText, sourceNode) {
    ...
    // 貼り付けを始める前に全個体を作り切る（＝魔物知識の値もここで確定する）
    const { records, knowledgeText } =
      buildVCallRecords(baseRec, baseName, initiative, count, shardCount, demonizeStage);

    // 従来と同じ 1000ms 後にコマンド行を編集保存する。
    // 保存する文字列が '' なら実質削除（従来どおり）、魔物知識が取れていれば
    // その行をお知らせテキストに差し替える。
    if (sourceNode) scheduleSelfEdit(sourceNode, knowledgeText, 1000);

    (async () => {
      await pasteVCallRecords(records, shardCount, demonizeStage);
    })();
  }
