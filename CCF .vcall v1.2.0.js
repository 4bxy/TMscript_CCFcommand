 ...
 * - v1.2.0: 召喚完了後に「魔物知識の値／弱点値(弱点内容)」をチャットへ自動投稿。
...
(function () {
  ...
  async function runVCallSequence(baseRec, baseName, initiative, count, shardCount, demonizeStage) {
    ...
    // 全個体の召喚が終わってから、まとめて魔物知識をチャットへ投稿する
    // （召喚は右クリック貼り付けなので、途中でチャット入力欄へフォーカスを移すと事故る）
    ...
    if (knowledgeLines.length > 0) {
      postChatMessage(knowledgeLines.join('\n'));
    } else {
      console.warn(LOG, '魔物知識の値がmemoから取得できなかったため投稿をスキップ:', baseName);
    }
  }
